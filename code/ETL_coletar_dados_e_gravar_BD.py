...
output_files = Path("D:/PythonProjects/ETLReceita/Dados_RFB")
extracted_files = Path("D:/PythonProjects/ETLReceita/Extracted_files")
...
#GABI
Files = ["F.K03200$Z.D20312.MOTICSV.zip","F.K03200$Z.D20312.CNAECSV.zip"]
...
#%%
# Creating directory to store the files being downloaded:
...
for l in Files:
    ...
    print(str(i_l) + ' - ' + l)
    url = dados_rf+l
    wget.download(url, out=os.path.join(output_files, l),bar=bar_progress)
...
#wget.download(Layout, bar=bar_progress)
...
for l in Files:
    try:
        ...
        print(str(i_l) + ' - ' + l)
        with zipfile.ZipFile(os.path.join(output_files, l), 'r') as zip_ref:
...
#%%
# Quebra dos arquivos em partes menores de acordo com parametro geral ->tamanho_das_partes
def quebra_arquivo(file_path):
    file_lenght = sum(1 for line in open(file_path, "r", encoding='ANSI'))
...
print(arquivos_empresa)
print(arquivos_estabelecimento)
print(arquivos_socios)
print(arquivos_simples )
print(arquivos_cnae)
print(arquivos_moti)
print(arquivos_munic) 
print(arquivos_natju)
print(arquivos_pais)
print(arquivos_quals)
...
for e in range(0, len(arquivos_empresa)):
    ...
    partes = quebra_arquivo(extracted_file_path)
...
# Drop table antes do insert
if len(arquivos_estabelecimento) > 0:
